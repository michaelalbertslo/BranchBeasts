...
function OutfitGen() {
  ...
  const prevHat = () => {
    if (hats.length === 0) return;
    setHatIndex(
      (prev) => (prev - 1 + hats.length) % hats.length
    );
  };
  const toggleHatKeep = () => {
  ...
  const prevJacket = () => {
    if (jackets.length === 0) return;
    setJacketIndex(
      (prev) => (prev - 1 + jackets.length) % jackets.length
    );
  };
  ...
  const prevShirt = () => {
    if (shirts.length === 0) return;
    setShirtIndex(
      (prev) => (prev - 1 + shirts.length) % shirts.length
    );
  };
  ...
  const prevPants = () => {
    if (pants.length === 0) return;
    setPantsIndex(
      (prev) => (prev - 1 + pants.length) % pants.length
    );
  };
  ...
  const prevSocks = () => {
    if (socks.length === 0) return;
    setSocksIndex(
      (prev) => (prev - 1 + socks.length) % socks.length
    );
  };
  ...
  const prevShoes = () => {
    if (shoes.length === 0) return;
    setShoesIndex(
      (prev) => (prev - 1 + shoes.length) % shoes.length
    );
  };
  ...
  //randomizer
  const randomizeAll = () => {
    if (!hatKeep && !hatNull && hats.length)
      setHatIndex(Math.floor(Math.random() * hats.length));
    if (!jacketKeep && !jacketNull && jackets.length)
      setJacketIndex(
        Math.floor(Math.random() * jackets.length)
      );
    if (!shirtKeep && !shirtNull && shirts.length)
      setShirtIndex(Math.floor(Math.random() * shirts.length));
    if (!pantsKeep && !pantsNull & pants.length)
      setPantsIndex(Math.floor(Math.random() * pants.length));
    if (!socksKeep && !socksNull && socks.length)
      setSocksIndex(Math.floor(Math.random() * socks.length));
    if (!shoesKeep && !shoesNull && shoes.length)
      setShoesIndex(Math.floor(Math.random() * shoes.length));
  };
  ...
  const saveOutfit = async () => {
    const hatObj = hatNull
      ...
      : hats.length > 0
        ? {
            id: hats[hatIndex]._id,
            image: hats[hatIndex].image_url
          }
        : { id: null, image: null };
    ...
    const jacketObj = jacketNull
      ...
      : jackets.length > 0
        ? {
            id: jackets[jacketIndex]._id,
            image: jackets[jacketIndex].image_url
          }
        : { id: null, image: null };
    ...
    const shirtObj = shirtNull
      ...
      : shirts.length > 0
        ? {
            id: shirts[shirtIndex]._id,
            image: shirts[shirtIndex].image_url
          }
        : { id: null, image: null };
    ...
    const pantsObj = pantsNull
      ...
      : pants.length > 0
        ? {
            id: pants[pantsIndex]._id,
            image: pants[pantsIndex].image_url
          }
        : { id: null, image: null };
    ...
    const socksObj = socksNull
      ...
      : socks.length > 0
        ? {
            id: socks[socksIndex]._id,
            image: socks[socksIndex].image_url
          }
        : { id: null, image: null };
    ...
    const shoesObj = shoesNull
      ...
      : shoes.length > 0
        ? {
            id: shoes[shoesIndex]._id,
            image: shoes[shoesIndex].image_url
          }
        : { id: null, image: null };
      ...
      <div className="grid grid-cols-5 items-center gap-2">
        ...
        <button
          ...
          className={`text-sm leading-none px-1 py-2 rounded-xl hover:scale-105 ${
            hatKeep
              ? "bg-gradient-to-br from-[#daffa0] via-[#9fe11d] to-[#daffa0] shadow-glass shadow-2xl backdrop-blur-sm border border-white/5"
              : "bg-gradient-to-br from-[#9fe11d] via-gray-200 to-[#9fe11d] shadow-glass shadow-2xl backdrop-blur-sm border border-white/50"
          }`}>
          {hatKeep ? "Held" : "Keep"}
        </button>
        {/* Left arrow */}
        ...
        <button
          ...
          className={`text-sm leading-none px-1 py-2 rounded-xl hover:scale-105 ${
            hatNull
              ? "bg-gradient-to-br from-[#F5A623] via-[#F55D23] to-[#F5A623] shadow-glass shadow-2xl backdrop-blur-sm border border-white/5"
              : "bg-gradient-to-br from-[#F5A623] via-gray-200 to-[#F5A623] shadow-glass shadow-2xl backdrop-blur-sm border border-white/5"
          }`}>
